    ...
    switch (evt->type()) {
      case QEvent::Type::ChildAdded: {
        if (watchedObject == _comboBox) {
          ...
          // Compare against the view we already reference instead of calling
          // view() again: this event fires while the popup view is mid-construction,
          // so re-invoking view() re-enters the constructor and overflows the stack.
          if (child == _view) {
  ...
  bool eventFilter(QObject* watchedObject, QEvent* evt) override {
    switch (evt->type()) {
      ...
      case QEvent::ChildAdded: {
        const auto* childEvent = static_cast<QChildEvent*>(evt);
        const auto* child = childEvent->child();
        // Compare against the child directly instead of calling _comboBox->view():
        // this event can fire while the popup view is mid-construction, and calling
        // view() again would re-enter the constructor and overflow the stack.
        if (qobject_cast<const QAbstractItemView*>(child) != nullptr) {
          if (auto* qlementine = qobject_cast<QlementineStyle*>(_comboBox->style())) {
            if (isDefaultItemDelegate(_comboBox->itemDelegate())) {
              _comboBox->setItemDelegate(new ComboBoxDelegate(_comboBox, *qlementine));
            }

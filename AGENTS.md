...
- **Test:** `ctest --test-dir build`. **62 tests** across `core/`, `gui/tests/` (`build/`), and the Vulkan-tests dir (`build/Vulkan-tests/`). All pass on this machine except `scrub_bench_vulkan_test` + `visual_render_parity_test`, which are SKIP-as-fail WIP stubs owned by the Vulkan phases (P-C/P-D) — treat those two as disabled until the kernel lands, not as regressions. **This paragraph was rewritten from a stale "35 tests" 2026-09-14; it was already wrong when found** — re-check with `ctest -N` before quoting:
  - **Core (24, all under `core/tests/`):** `roundtrip` (edit-op + project JSON round-trip; an edge in this test used to SEGFAULT on a stale timeline model until `EqBand::operator==` was added — it now passes), `export_sweep` (every valid codec×container combo; skip=2 if no libx264), `scrub_bench` (p95 scrub-preview latency budget; skip=2 if no libx264), `colorsci`, `graph`, `graph_edit`, `composite`, `op`, `lut` (Resolve-style 3D grade-LUT bake: identity byte-exact round-trip, gain byte-exact vs the evaluator, grid-vs-evaluator parity at gridpoints, empty/unwired → null), `gpu_grade` (pins the fused CUDA grade kernel's law — `nv12GradeResize` — by host-side mirror, incl. identity/limited-range/CPU-export parity, no device needed), `wheels_ui`, `curves`, `histogram`, `visual_render`, `voice_isolation`, `vram_leak` (VRAM regression gate: NVDEC decode + host download sampling `cudaMemGetInfo`; fail if free VRAM drains >256 MB), `clip_rate` (per-clip speed law/pitch + WSOLA engine), `equalizer` (EqBank/EqBand laws — **green again** after the Inspector-EQ WIP settled), `vaapi_encode` (VAAPI rc/preset mapping, headless no-device), `vaapi_driver` (VAAPI registry + surface-ownership contract, headless no-device), `gpu_select` (GPU/vendor-select logic), `qsv_encode` (QSV rc mapping), `vaapi_enc` (device-bound round-trip on the real render node — skip=2 without a working VAAPI encoder), `vaapi_enc_bench` (VAAPI speed sweep at the deliver target: hevc/h264 rows, decodes each output in-process before reporting PASS).
  - **GUI-headless (14, in `gui/tests/`, `build/`):** `sync_constants_test`, `timeline_decoder_test`, `transition_bake_test`, `audio_pipeline_test`, `sonicsync_test`, `av_reanchor_test`, `timeline_snap_test`, `timeline_selection_test`, `timeline_drag_test`, `transition_handle_editor_test`, `audio_targets_test`, `timeline_volume_line_test`, `deliver_settings_model_test`, `source_preview_model_test`.
  - **GUI Qt-linked (4, in `gui/tests/qt/`):** `volume_line_drag_qt_test`, `waveform_placement_qt_test`, `wheel_panel_roundtrip_qt_test`, `theme_roundtrip_qt_test`.
  - **Vulkan-tests (10, in `build/Vulkan-tests/`):** `vk_probe_test` (SKIP without a pe�vulkan[1] device), `gpu_grade_vulkan_test`, `queue_matrix_test`, `video_profiles_test`, `formats_test`, `interop_contract_test`, `vram_leak_vulkan_test`, `export_sweep_vulkan_test`, plus the two SKIP stubs above.
...
- `cuda_convert.cu`/`.hpp` — CUDA kernels feeding NVENC directly: `rgbaToNV12`, `nv12Resize` (+ `nv12GradeResize` fusion), and `nv12TitleBlend`, which fuses the per-clip **title sprite** (`title::raster_title_sprite`, premultiplied-RGBA8 box) over the already-faded resize/grade output inside the same kernel launch. `title_sprite_upload`/`title_sprite_free` manage the device sprite (per-`Clip*` cached in the exporter via `GpuTitleCache`; sprite upload failure falls back to the CPU compositor). Compile-guarded under `CANVAS_HAVE_CUDA`; all callers guard with `cuda_available()`. The export fast path now keeps titles on the GPU (benchmark: "baseline + subtitle/fades" row ~283 fps ≈ the 284 fps baseline, vs 59 fps when it bailed to the CPU compositor); the VAAPI/CPU paths still composite titles on the CPU for pixel parity.
...
- `src/features/`:
  ...
  - `project/ProjectActions.cpp` — import/new/open/save (`*.ncs`, legacy `.ehproj` files still open via the loader's JSON-key fallback), recent files (QSettings `"recentProjects"`, cap 10).

...
- `src/UX/InspectorAudio.cpp` — the Inspector's Audio tab (Volume/Pan, Pitch, Speed Change, **interactive EQ**, AI Voice Isolation). The EQ panel (`EqGraphWidget`) is a **dual-mode** interactive editor with a segmented **Curve | EasyEffects** view toggle. Curve mode is a true draggable-node graph: drag = freq+gain, Shift = freq-only, Ctrl/Alt = gain-only, wheel = Q (log 1.15×, debounced into one undoable edit), click selects, double-click toggles per-band `EqBand.enabled`; EasyEffects mode is six vertical gain-fader columns (drag a column = that band's gain, wheel = Q, double-click = bypass, LP/HP pinned to 0 dB). Row widgets echo the graph live and each settled gesture commits once via `apply_inspector_audio_processing`. Callbacks are `std::function` (no signals/moc).
...
- `src/features/`:
  ...
  - `playback/audio_pipeline.hpp/.cpp` — **headless.** Owns the per-media `AudioDecoder` set, playhead↔sample math, A/V-sync run anchors, feed watermarks, scrub-audio grains/reposition feed, and the mixed-source write path (stretching per-clip speed via the WSOLA `TimeStretchBank` — pitch-preserving — then applying per-clip volume/pan via `audio_mix.hpp` and the fade envelopes via `audio_fade.hpp` before summing). Talks to the device only through `AudioSink` — never a concrete `AudioOutput` — so it can run against a fake sink in tests. FROZEN API. **Always-on per-stage sample audit:** every mix tick scans decoder/WSOLA/RNNoise/EQ/lane/mix for peak, non-finite, and the single-sample-step "pop" shape; on a hit it logs `[audio:audit] STAGE=<stage> POP|BLAST|NONFINITE ...` with full blame context (clip/media/mediaSample/dsp) and WAV-dumps ±64 samples to `/tmp/canvas_audio_audit/`. Also emits a 1 Hz `TICK` trace (`mixPk/decMs/mixMs/xruns`), `SLOW` warnings when a mix step outlives its 33 ms window (DAC underrun click), and `XRUN delta=` lines from `AudioSink::stat_xruns()`.
  - `playback/audio_sink.hpp` — **headless.** Abstract interface (`open/close/write_float/flush/reposition_enqueue/audible_position_frames/stat_written_frames/stat_xruns/...`) that `AudioOutput` implements 1:1; exists purely so `AudioPipeline` never has to see ALSA/PipeWire.

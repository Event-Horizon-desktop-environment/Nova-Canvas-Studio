...
public:
    ...
    void paint(QPainter* p, const QStyleOptionViewItem& opt,
        ...
        // Video+audio media split the well: the video frame rides the top and a
        // live audio-spectrum strip (generated by ThumbnailService) the bottom.
        ...
        if (hybrid) {
            ...
            if (pm.isNull()) {
                // Tinted placeholder for the top well until the frame lands.
                ...
                // Scale-to-fill the top band, radius-clipped with the well.
            ...
            // Hairline seam + the audio-spectrum strip.
        ...
        // Hover-scrub playhead (Live Media Preview): a soft-edged 2px accent
        // stroke across the well at the pointer's x, with a little top cap so it
        // reads as a playhead rather than a seam.
...
void MediaPoolWidget::mouseMoveEvent(QMouseEvent* event) {
    // Hover-scrub only while no button is held: a pressed drag is a media
    // drag (or tile selection) and must not run the Live Media Preview skim.
...
void MediaPoolWidget::update_scrub_from_mouse(const QPoint& viewport_pos) {
    ...
    double frac = 0.0;
    if (item) {
        // Map the pointer's x to a 0..1 fraction of the tile body (the same
        // rect the delegate insets by the 2px card margin), clamped so the
        // playhead never leaves the well.
